...
from pytest_mysqlit.parsers.import_parser import ImportParser
...
class DbHelper:
    ...
    def exec_sql_file(conn, sql_file_path, global_path) -> None:
        ...
        for sql_command in sql_commands:
            ...
            if match_type == 'local':
                DbHelper.exec_sql_file(conn, sql_file, global_path)
            elif match_type == 'global':
                DbHelper.exec_sql_file(conn, os.path.join(global_path, sql_file), global_path)
            else:
